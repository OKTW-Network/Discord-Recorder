...
function startRecord(connection, channelConfig) {
    ...
    recvStream.on("data", (data, userID, timestamp, sequence) => {
        ...
        if (userDecoders[userID] == undefined) {
            userDecoders[userID] == true;
            ...
            outputMixer.addSource(global.discord.audio.playMixer[channelConfig.channelID])
            ...
            AudioUtils.generatePCMtoMP3Stream(userMixer).on("data", mp3Data => {
                ...
                if (global.discord.userMP3Buffer[channelConfig.channelID][userID].length > 1024) {
